...
var EventDispatcher = require('../events/event-dispatcher'),
    ...
    pluginHandler = require('../plugin/plugin'),
    path = require('path'),
    fs = require('fs'),
    _ = require('lodash');
...
var Launcher = EventDispatcher.define({
    ...
    loadPlugins:function(){
        _.forEach(pluginHandler.plugins,function(func){
            var toInject = func(environments);

            _.forEach(toInject,function(obj,key){
                inject.addObject(key, obj);
            });
        })
    },
    ...
    reset:function(){
        ...
        pluginHandler.plugins.length = 0;
        this._options = null;
